...
class Res101Encoder(nn.Module):
    ...
    def __init__(self, replace_stride_with_dilation=None, pretrained_weights='resnet101', dim=512):
        super().__init__()
        # using pretrained model's weights
        if pretrained_weights == 'deeplabv3':
            self.pretrained_weights = torch.load(
                r"checkpoints/deeplabv3_resnet101_coco-586e9e4e.pth", map_location='cpu')
        elif pretrained_weights == 'resnet101':
            self.pretrained_weights = torch.load(r"D:\github_code\RPT-main\checkpoints\resnet101-63fe2227.pth",
                                                 map_location='cpu')
        else:
        ...
        self.reduce1 = nn.Conv2d(1024, dim, kernel_size=1, bias=False)
        self.reduce2 = nn.Conv2d(2048, dim, kernel_size=1, bias=False)
...
class Res101Encoder_gmrd(nn.Module):
    """
    Resnet101 backbone from deeplabv3
    modify the 'downsample' component in layer2 and/or layer3 and/or layer4 as the vanilla Resnet
    """
