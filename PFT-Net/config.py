...
def cfg():
    ...
    eval_fold = 0  # (0-4) for 1-fold cross-validation
    ...
    exp_str = '_'.join(  # train/test_CHAOST2_cv
        [mode]
        + [dataset, ]
        + [f'cv{eval_fold}'])
...
def add_observer(config, command_name, logger):
    ...
    if config['mode']=='test':
        last_e_index = config['reload_model_path'].rfind('e')
        epoch = config['reload_model_path'][last_e_index:-4]
        observer = FileStorageObserver.create(os.path.join(config['path']['log_dir'], exp_name, epoch))
    else:
