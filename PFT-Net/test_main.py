...
def main(_run, _config, _log):
    ...
    _config['reload_model_path'] = r'epoch35.pth'

    checkpoint = torch.load(_config['reload_model_path'], map_location='cpu')
    model.load_state_dict(checkpoint['state_dict'], strict=False)
    model_total_epoch = checkpoint['epoch'] + 1
    _log.info(f'Loading the model of epoch {model_total_epoch} successfully')
    ...
    for label_val, label_name in labels.items():
        ...
        with torch.no_grad():
            ...
            for i, sample in enumerate(test_loader):
                ...
                query_label = sample['label'].float().squeeze(dim=0).cuda()  # C x H x W
                ...
                for sub_chunck in range(_config['n_part']):
                    ...
                    query_label_s = query_label[idx_[sub_chunck]:idx_[sub_chunck + 1]]  # C' x 1 x H x W
                    query_pred_s = []
                    for i in range(query_image_s.shape[0]):
                        _pred_s = model([support_image_s], [support_fg_mask_s], [query_image_s[[i]]], query_label_s[[i]], train=False)  # C x 1 x H x W
                ...
                # Record scores.
                scores.record(query_pred, query_label.cpu())
    ...
    _log.info(f'====================================================Final model of epoch {model_total_epoch} test results====================================================')
